...
import { Archive, ArrowLeft, FilePlus2, FileText, Maximize2, Minimize2, Search, Upload, X } from "lucide-react";
...
export default function OrganizationFolderPage() {
  ...
  return <div className="min-h-full mx-auto max-w-[1650px] space-y-6 bg-slate-50 p-6 pb-10">
    ...
    {showUpload && <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/30 p-4 backdrop-blur-sm"><form onSubmit={submitUpload} className="w-full max-w-lg rounded-3xl bg-white p-6 shadow-2xl"><div className="flex items-start justify-between"><div><p className="text-xs font-bold uppercase tracking-[0.18em] text-brand-pink">Organizational file</p><h2 className="mt-1 text-xl font-bold text-slate-900">Add document</h2></div><button type="button" onClick={() => setShowUpload(false)} className="rounded-full p-2 text-slate-400 hover:bg-pink-50 hover:text-brand-pink"><X /></button></div><label className="mt-5 block"><span className="label">File</span><span className="flex cursor-pointer items-center gap-3 rounded-xl border border-dashed border-brand-pink/40 bg-pink-50/50 px-4 py-5 text-sm font-semibold text-brand-text"><Upload className="h-5 w-5" />{file?.name ?? "Choose a file from your computer"}<input required type="file" onChange={(event) => setFile(event.target.files?.[0] ?? null)} className="hidden" /></span></label><div className="mt-4 flex items-end gap-2"><label className="min-w-0 flex-1"><span className="label">Document type</span><ThemedSelect value={typeId} onChange={setTypeId} placeholder="Select document type" options={(types.data ?? []).map((type: any) => ({ value: String(type.id), label: type.name }))} /></label><InlineDocumentTypeCreator onCreated={(type) => setTypeId(String(type.id))} /></div><div className="mt-6 flex justify-end gap-2"><button type="button" onClick={() => setShowUpload(false)} className="rounded-xl px-4 py-2.5 font-semibold text-slate-500">Cancel</button><button disabled={upload.isPending} className="rounded-xl bg-gradient-to-br from-brand-text to-brand-pink px-4 py-2.5 font-semibold text-white">{upload.isPending ? "Uploading..." : "Upload document"}</button></div></form></div>}
    {viewing && <div className={`fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 backdrop-blur-sm ${viewerFullscreen ? "" : "p-4"}`}><div className={`flex w-full flex-col overflow-hidden bg-white shadow-2xl ${viewerFullscreen ? "h-screen max-w-none rounded-none" : "max-h-[90vh] max-w-5xl rounded-3xl"}`}><div className="flex items-center justify-between border-b border-slate-100 p-4"><div><h2 className="font-bold text-slate-900">{viewing.name}</h2><p className="text-xs text-slate-400">{viewing.document_type}</p></div><div className="flex items-center gap-2"><button type="button" onClick={() => window.print()} className="rounded-xl border border-slate-200 px-3 py-2 text-sm font-semibold text-slate-600">Print</button><button type="button" onClick={() => api.downloadDocument(viewing.id)} className="rounded-xl bg-brand-pink px-3 py-2 text-sm font-semibold text-white">Download</button><button type="button" onClick={() => setViewerFullscreen((current) => !current)} aria-label={viewerFullscreen ? "Exit full screen" : "Open full screen"} className="rounded-xl border border-slate-200 p-2 text-slate-500 hover:border-brand-pink hover:text-brand-pink">{viewerFullscreen ? <Minimize2 className="h-4 w-4" /> : <Maximize2 className="h-4 w-4" />}</button><button type="button" onClick={() => { setViewing(null); setViewerFullscreen(false); }} className="rounded-xl p-2 text-slate-400 hover:bg-pink-50 hover:text-brand-pink"><X /></button></div></div><div className="min-h-0 flex-1 overflow-hidden bg-slate-50 p-5">{useTestData ? <div className="mx-auto max-w-2xl rounded-2xl bg-white p-8 shadow-sm"><FileText className="h-10 w-10 text-brand-pink" /><h3 className="mt-5 text-xl font-bold text-slate-900">Test document preview</h3><p className="mt-2 leading-7 text-slate-600">{viewing.description || "This is mock document content for development."}</p><p className="mt-6 text-sm text-slate-400">{viewing.name} · {viewing.mime_type}</p></div> : <iframe title={viewing.name} src={`${(process.env.NEXT_PUBLIC_ODOO_URL || "").replace(/\/$/, "")}/document-management/document/${viewing.id}/preview`} className="block h-full min-h-[65vh] w-full pointer-events-auto rounded-2xl border border-slate-200 bg-white" />}</div></div></div>}
  </div>;
}
