...
export default function RolesPage({ embedded = false }: { embedded?: boolean }) {
  ...
  useEffect(() => {
    if (!selectedEmployeeId && members.length === 1) {
      setSelectedEmployeeId(members[0].employee_id);
    }
  ...
  async function handleSave() {
    ...
    try {
      ...
      setNotice("Document Management roles updated successfully.");
    } catch (error) {
      setNotice(
        error instanceof Error ? error.message : "Roles could not be updated.",
      );
    }
  }
      ...
      {!embedded ? (
        <div className="roles-admin-header">
          <div>
            ...
            <h1>Document Management roles</h1>
            <p>
              Assign Manager and Administrator responsibilities for Document Management.
              Baseline User access is granted automatically to internal employees.
            </p>
          </div>
          <button
            type="button"
        ...
        <div className="roles-admin-toolbar">
          <p>Assign Manager and Administrator roles for Document Management.</p>
          <button
            type="button"
            className="primary-button"
            onClick={handleSave}
        ...
        <section className="roles-admin-panel">
          ...
          <div className="roles-admin-list">
            {membersQuery.isLoading && (
              <p className="roles-admin-empty">Searching employees...</p>
            )}
            {!membersQuery.isLoading && !members.length && (
              <p className="roles-admin-empty">No employees match this search.</p>
            )}
            {members.map((member) => (
              <button
        ...
        <div className="roles-admin-detail">
          {!selectedMember && (
            <p className="roles-admin-empty">
              Select an employee to review and assign Document Management roles.
            </p>
          )}
          {selectedMember && (
            <>
              <section className="roles-admin-panel">
                ...
                {!selectedMember.has_login && (
                  <div className="roles-admin-warning">
                    ...
                    <span>
                      This employee does not have a linked Odoo user account. Create a user
                      in Odoo Settings before assigning roles.
                    </span>
                  </div>
                )}
              </section>
                  ...
                  assignableRoles.map((role) => (
                    <div key={role.role_key} className="roles-admin-toggle-row">
                      ...
                      <button
                        ...
                        onClick={() =>
                          handleToggle(role.role_key, !activeMember.roles[role.role_key])
                        }
                      />
                    </div>
                  ))}
