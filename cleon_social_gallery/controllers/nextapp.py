...
MODULE = "cleon_social_gallery"
NEXTAPP_STATIC_DIR = "static/src/nextapp"


class SocialGalleryNextAppController(http.Controller):
    @staticmethod
    ...
    @http.route(
        ["/social-gallery", "/social-gallery/<path:subpath>"],
        type="http",
        auth="user",
        sitemap=False,
    )
        ...
        if html is None:
            ...
            if html is None:
                return request.make_response(
                    ...
                    + "`cd cleon_social_gallery/next-app && npm run deploy`.",
    ...
    def api_me(self, **kwargs):
        try:
            ...
            _logger.exception("Error in Social Gallery /api/me: %s", error)
