...
class GalleryRoleService(models.AbstractModel):
    _name = "gallery.role.service"
    _description = "Social Gallery role assignment service"

    @api.model
    def _require_system_admin(self):
        if not self.env.user.has_group("base.group_system"):
            raise AccessError(_("Only system administrators can manage Social Gallery roles."))

    @api.model
    def _definitions(self):
        self._require_system_admin()
        self.env["gallery.role.definition"].sync_registry()
        rows = self.env["gallery.role.definition"].search([])
        return [
            {
                "id": row.id,
    ...
    def _serialize_member(self, employee):
        ...
        roles = {}
        for definition in self.env["gallery.role.definition"].search([]):
    ...
    def _definition_for(self, role_key):
        definition = self.env["gallery.role.definition"].search(
            [("role_key", "=", role_key)],
            limit=1,
        )
        if not definition:
    ...
    def _write_audit(self, user, employee, definition, action):
        self.env["gallery.role.assignment.audit"].sudo().create(
            {
                "user_id": user.id,
