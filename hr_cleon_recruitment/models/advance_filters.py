...
class advanceFiltersWizard(models.TransientModel):
    _inherit = "advance.filters.wizard"

    jobs_id = fields.Many2one('hr.job')
    date_from = fields.Date()
    date_to = fields.Date()
    min_number = fields.Integer()
    max_number = fields.Integer()
    advLocation = fields.Char('Location')
    jobs_id = fields.Many2one('hr.job')
    hiring_team_id = fields.Many2one(
            'hr.employee',
            string='Hiring team',
            store=True,
        )
        
    branch_id = fields.Many2one('multi.branch', string='Location')
