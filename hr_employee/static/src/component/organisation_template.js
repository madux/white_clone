...
export class OrganisationListRenderer extends ListRenderer {
  setup() {
    ...
    onWillStart(async () => {
      const menus = await this.orm.call('ir.ui.menu', 'get_hr_parent_menus', [
        'HRCORE',
      ]);
  ...
  async openMenu(menu) {
    // ev.preventDefault();
    const [actionId, menuId] = await this.orm.call(
      'hr.employee',
...
registry
  .category('views')
  .add('organisation_list', CustomOrganisationDashboardListView);
