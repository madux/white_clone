...
export class LeaveReportsPage extends Component {
    ...
    renderCharts() {
        ...
        this.charts.type = new Chart(typeCanvas.getContext("2d"), { type: "doughnut", data: { labels: types.map(item => `${item.name}: ${item.days}d`), datasets: [{ data: types.map(item => item.days), backgroundColor: types.map(item => item.color), borderWidth: 2 }] }, options: { responsive: true, maintainAspectRatio: false, cutout: "55%", plugins: { legend: { position: "bottom", labels: { usePointStyle: true, boxWidth: 8 } } } } });
