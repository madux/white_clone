    ...
    setup() {
        ...
        onWillUpdateProps((nextProps) => {
            if (this.props.people !== nextProps.people) {
                // When props change, we must update D3 graph
                if (this.d3Loaded && window.d3) {
                    // Use a microtask/timeout to let any DOM changes settle, though for SVG manipulation
                    // inside the component we can just rebuild it immediately with nextProps.people
                    // Wait, renderGraph uses this.props.people. But in onWillUpdateProps, this.props is not yet updated!
                    // So we should schedule renderGraph for after the update, or pass nextProps.people to it.
