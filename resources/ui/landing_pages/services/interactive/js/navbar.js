...
/* Returns path from current page to the shared root folder
   (the directory that contains both 'services/' and 'industries/') */
function chrGetRootPath() {
  const path = window.location.pathname;
  const parts = path.split('/').filter(Boolean);
  const landingIdx = parts.indexOf('landing_pages');
  if (landingIdx !== -1) {
    const depth = parts.length - landingIdx - 2;
    return depth > 0 ? '../'.repeat(depth) : '';
  }
  let idx = parts.indexOf('interactive');
  if (idx === -1) idx = parts.indexOf('interactive-components');
  if (idx === -1) return '';
...
const CHR_SVC_DATA = {
  'core-hr': {
    ...
    modules: [
      { id: 'hr-admin', icon: 'fa-building', iconClass: 'chr-svc-card__icon--pink', title: 'HR Administration', badge: 'core', badgeClass: 'chr-svc-card__badge--core', desc: 'Complete employee lifecycle management', url: 'all-modules/core-hr-operations/hr-admin/hr-administration.html' },
  ...
  'payroll': {
    ...
    modules: [
      { id: 'payroll', icon: 'fa-money', iconClass: 'chr-svc-card__icon--pink', title: 'Payroll & Remittance', desc: 'Automated payroll and tax filing', url: 'all-modules/payroll-and-finnance/payroll-and-remittance/payroll-and-remittance.html' },
      { id: 'hr-finance', icon: 'fa-bar-chart', iconClass: 'chr-svc-card__icon--blue', title: 'HR Finance Management', desc: 'Budgeting, expenses, and financial reporting', url: 'all-modules/payroll-and-finnance/hr-finance-management/hr-finance-management.html' },
      { id: 'cleontime', icon: 'fa-clock-o', iconClass: 'chr-svc-card__icon--green', title: 'CleonTime — Claims', desc: 'Timesheets, claims, and overtime management', url: 'all-modules/payroll-and-finnance/cleon-time-claims/cleon-time-claims.html' },
      { id: 'expense-mgmt', icon: 'fa-credit-card', iconClass: 'chr-svc-card__icon--teal', title: 'Expense Management', desc: 'Employee expense tracking and reimbursements', url: 'all-modules/payroll-and-finnance/expense-management/expense-management.html' },
      { id: 'petty-cash', icon: 'fa-briefcase', iconClass: 'chr-svc-card__icon--orange', title: 'Petty Cash', desc: 'Petty cash management and tracking', url: 'all-modules/payroll-and-finnance/petty-cash/petty-cash.html' },
      { id: 'chart-of-accounts', icon: 'fa-book', iconClass: 'chr-svc-card__icon--purple', title: 'Chart of Accounts', desc: 'Financial account structure and management', url: 'all-modules/payroll-and-finnance/chart-of-accounts/chart-of-accounts.html' },
      { id: 'journal-entries', icon: 'fa-file-text-o', iconClass: 'chr-svc-card__icon--pink', title: 'Journal Entries', desc: 'Manual journal entries and adjustments', url: 'all-modules/payroll-and-finnance/journal-entries/journal-entries.html' }
    ]
  },
  'compliance': {
    ...
    modules: [
      { id: 'kyc', icon: 'fa-id-card-o', iconClass: 'chr-svc-card__icon--pink', title: 'Verification & KYC', desc: 'Employee verification and background screening', url: 'all-modules/compliance-and-governance/vkyc/vkyc.html' },
      { id: 'internal-control', icon: 'fa-shield', iconClass: 'chr-svc-card__icon--blue', title: 'Internal Control & Compliance', desc: 'Audit trails and compliance monitoring', url: 'all-modules/compliance-and-governance/internal-control/internal-control.html' },
      { id: 'audit-trail', icon: 'fa-history', iconClass: 'chr-svc-card__icon--green', title: 'Audit Trail', desc: 'Complete audit trail and activity logging', url: 'all-modules/compliance-and-governance/audit-trail/audit-trail.html' }
    ]
  },
  'client': {
    ...
    modules: [
      { id: 'client-mgmt', icon: 'fa-handshake-o', iconClass: 'chr-svc-card__icon--pink', title: 'Client Management', desc: 'Client relationship and account management', url: 'all-modules/client-business-operations/client-management/client-management.html' },
      { id: 'contracts', icon: 'fa-file-text', iconClass: 'chr-svc-card__icon--blue', title: 'Contract Management', desc: 'Contract lifecycle and billing', url: 'all-modules/client-business-operations/contract-management/contract-management.html' }
    ]
  },
  'platform': {
    ...
    modules: [
      { id: 'hr-ai', icon: 'fa-bolt', iconClass: 'chr-svc-card__icon--pink', title: 'AI Integration Engine', desc: 'AI-powered HR insights and automation', url: 'all-modules/platform-intelligence/ai-integration-engine/ai-integration-engine.html' },
      { id: 'analytics', icon: 'fa-line-chart', iconClass: 'chr-svc-card__icon--blue', title: 'Reporting & Analytics Platform', desc: 'Comprehensive HR analytics and dashboards', url: 'all-modules/platform-intelligence/reporting-analytics/reporting-analytics.html' },
      { id: 'notifications', icon: 'fa-bell', iconClass: 'chr-svc-card__icon--green', title: 'Notification & Communication Engine', desc: 'Multi-channel notifications and alerts', url: 'all-modules/platform-intelligence/notification-engine/notification-engine.html' },
      ...
      { id: 'gems', icon: 'fa-globe', iconClass: 'chr-svc-card__icon--orange', title: 'Global Employee Management System (GEMS)', desc: 'Multi-country employee management', url: 'all-modules/platform-intelligence/gems/gems.html' }
    ]
  },
  'marketplace': {
    ...
    modules: [
      { id: 'marketplace', icon: 'fa-shopping-cart', iconClass: 'chr-svc-card__icon--pink', title: 'Cleon Marketplace & Services Ecosystem', desc: 'API integrations, third-party services, payroll partners, and HR vendors', url: 'all-modules/marketplace-ecosystem/marketplace/marketplace.html' }
    ]
  }
};
...
/* ── INDUSTRIES DATA ─────────────────────────────────────── */
const CHR_IND_DATA = [
  { icon: 'fa-heartbeat', iconClass: 'chr-ind-card__icon--pink', title: 'Healthcare & HMO', desc: 'Manage hospitals, HMOs, medical staff, compliance, shift scheduling, and healthcare payroll.', url: 'industries/interactive-components/healthcare-hmo/healthcare-hmo.html' },
...
function renderIndContent() {
  const grid = document.querySelector('.chr-ind-grid');
  if (!grid) return;

  grid.innerHTML = CHR_IND_DATA.map(ind => {
    const linkHref = ind.url ? chrGetRootPath() + ind.url : '#';
    return `
      <a href="${linkHref}" class="chr-ind-card" style="display:block;text-decoration:none;color:inherit;">
        <div class="chr-ind-card__icon ${ind.iconClass}"><i class="fa ${ind.icon}"></i></div>
        <div class="chr-ind-card__body">
          <div class="chr-ind-card__title">${ind.title}</div>
          <div class="chr-ind-card__desc">${ind.desc}</div>
        </div>
      </a>
    `;
  }).join('');
